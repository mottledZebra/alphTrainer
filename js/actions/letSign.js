var letSign = function(args) {
  ...
  var pageHeader = [
    ...
    },
    {
      ...
      content: 'Нажмите на кнопку, соответствующую начертанию буквы'
    },
  ];
  ...
  var question = {
    tag: 'img',
    attributes: {
      'src': 'img/' + language + '/' + answersArray[questionIndex].single    
    }
  };
  ...
  answersArray.forEach(function(answer) { 
    var letterVariant = getLetterVariant(answer, features);
    ...
    var button = replaceTemplate(answerTemplate, [++counter, answer.name + ' ' + getLegend(letterVariant)]);
  ...
  function redrawPage(questionIndex, answersArray) {
    ...
    document.querySelector('.question img').setAttribute('src', 'img/' + language + '/' + answersArray[questionIndex][questionLetterVariant]);
    ...
    document.querySelectorAll('.btn').forEach(function(item) {
      ...
      item.innerHTML = answersArray[i++].name + ' ' + getLegend(letterVariant);
    });
  }
}
